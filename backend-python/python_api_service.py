...
class ProcessingRequest(BaseModel):
    api_url: str = "http://localhost:5010/api/material-all"
    qdrant_url: str = "http://localhost:6333"
    model_name: str = "sentence-transformers/all-mpnet-base-v2"
    collection_name: str = "material_vectors"
    batch_size: int = 20
    max_workers: int = 5
...
async def initialize_processor():
    ...
    try:
        ...
        try:
            from shared.scripts.warehouse.material_processor import MaterialVectorProcessor
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting up Material Vector Processing API...")
    await initialize_processor()
    yield
    # Shutdown
    logger.info("Shutting down Material Vector Processing API...")
    await cleanup_processor()

# Create FastAPI app with lifespan handler
app = FastAPI(
    title="Material Vector Processing API", 
    version="1.0.0",
    lifespan=lifespan
)
...
async def process_materials_sync(request: ProcessingRequest):
    """Synchronous processing endpoint (use with caution for large datasets)"""
    if initialization_error:
        raise HTTPException(
            status_code=500, 
            detail=f"System not properly initialized: {initialization_error}"
